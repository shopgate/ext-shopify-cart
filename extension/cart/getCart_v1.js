...
module.exports = async function (context, input) {
  ...
  return Tools.getCurrentCartId(context).then(() => {
    /**
    ...
    return createCart(shopifyRequestErr, shopifyCartData).then((cart) => {
      return Tools.setCurrentCartId(context, cart.id).then(() => {
        return cart
      }).catch((err) => {
        return err
      })
