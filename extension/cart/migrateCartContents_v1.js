...
const updateCart = require('./../helper/updateCart')
const clearCart = require('./../helper/clearCart')
...
function migrateCartContents (context, sourceCartId, sourceCartLineItems, targetCartId, targetCartLineItems, cb) {
  ...
  clearCart(sourceCartId, context)
    .then(
      updateAndAdjustCart(updatedData, targetCartId, context)
    )
    .catch(err => {
      context.log.error(
   ...
   * @param {object} context
   */
  function updateAndAdjustCart (updatedData, targetCartId, context) {
