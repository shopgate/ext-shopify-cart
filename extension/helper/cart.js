...
const clearCart = async (sourceCartId, context) => {
  ...
  return shopifyApiRequest.put(`/admin/api/2022-10/checkouts/${sourceCartId}.json`, clearData)
}
...
const updateCart = async (updatedData, targetCartId, context) => {
  ...
  return shopifyApiRequest.put(`/admin/api/2022-10/checkouts/${targetCartId}.json`, updatedData)
}
...
async function fixCheckoutQuantities (checkoutCartItems, itemsToUpdate, error, cartId, context) {
  ...
  try {
    await shopifyApiRequest.put(`/admin/api/2022-10/checkouts/${cartId}.json`, productData)
  } catch (err) {
